...
DLLContainer<T>::DLLContainer(DLLContainer&& other) noexcept
    ...
    other.m_head = nullptr; other.m_tail = nullptr; other.m_size = 0;
}
...
void DLLContainer<T>::push_back(const T& value) {
    ...
    if (!m_tail) { m_head = m_tail = newNode; }
    else {
        m_tail->next = newNode;
        newNode->prev = m_tail;
        m_tail = newNode;
...
void DLLContainer<T>::erase(size_t index) {
    ...
    else m_head = toDelete->next;

    if (toDelete->next) toDelete->next->prev = toDelete->prev;
    else m_tail = toDelete->prev;
